...
export function renderUpdateModal(DOM) {
    if (!DOM.versionUpdateModal) return;

    const version = window.APP_VERSION || "v2.16.2";
    DOM.versionModalTitle.textContent = `${version} 업데이트 안내`;

    // 개발자 한마디
    DOM.devMessageContent.textContent = "여러분 새해 복 많이 받으세요.\n이번 업데이트는 많은 기능이 추가 되었습니다. 특히 중립이 사용하기 편한 기능이 많이 추가 되었습니다. 릴리즈 노트와 도움말을 참조해서 사용 부탁 드립니다.";

    // 업데이트 요약 목록
    const summaryItems = [
        "<strong>메뉴/입력 통합:</strong> 시간표와 스케줄러 명칭 정리 및 텍스트 모드 통합",
        "<strong>개인화 기능:</strong> 나만의 보스 젠 주기 설정 및 시간대별 전용 메모 지원",
        "<strong>스마트 내비게이션:</strong> PC 슬라이딩 메뉴 및 모바일 5개 핵심 탭 적용",
        "<strong>스케줄 자동화:</strong> 자정 기준 48시간 일정 자동 생성 및 업데이트 엔진",
        "<strong>안정성/데이터:</strong> 게임별 독립 작업 공간(입력 보존) 및 자가 복구 시스템",
        "<strong>공유/알림:</strong> 시간표 이미지 저장 기능 및 PiP 미니 알림창 시각화 개선"
    ];
