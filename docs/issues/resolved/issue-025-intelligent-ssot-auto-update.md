...
## 1. 개요 (Overview)
보스 스케줄의 48시간(오늘+내일) 윈도우를 항상 최신으로 유지하기 위해 정해진 시간(00:00, 12:00)에 SSOT를 자동으로 업데이트하는 메커니즘을 구현합니다.
...
2.  **트리거 시스템 구축**:
    ...
    *   `alarm-scheduler.js` (updateAppState): 타이머 워커의 1초 주기 TICK 마다 00/12시 기준점 통과 감시.
...
### 최종 결과
- ✅ 자정 및 정오가 되면 별도의 조작 없이도 내일 일정이 포함된 48시간 데이터로 자동 확장됩니다.
