...
3. **배포 전략 변경:** 배포가 간헐적으로 실패하는 문제를 해결하기 위해, 한글 파일명이 포함된 `.agent/workflows/` 폴더를 `.gitignore`에 추가하고 Git 추적에서 제외함. 이를 통해 로컬에서는 한글 명령어를 계속 사용하면서도 배포 서버에는 영향을 주지 않도록 조치.
4. **보안 조치:** 실수로 추적 중이던 `.env` 파일을 `git rm --cached`로 제거하고 `.gitignore`에 등록.
5. **불필요 리소스 정리:** Supabase, OneSignal 등 현재 프로젝트에서 사용하지 않는 외부 서비스 관련 코드와 환경 변수를 모두 삭제하여 사이트 경량화.
6. **검증:** 조치 후 배포가 다시 정상적으로 완료됨을 확인.
7. **지침 업데이트:** `GEMINI.md`에 해당 내용을 기록하여 향후 재발 방지.
