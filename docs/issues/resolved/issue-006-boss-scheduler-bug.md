...
애플리케이션 초기에는 보스 시간을 단순히 24시간 범위 내의 정렬된 목록으로 입력받았으며, 명시적인 날짜 구분 없이 시간만으로 보스 일정을 관리했습니다. 이 방식은 자정을 넘어가거나 특정 시간대에 걸쳐 보스가 등장할 때, 시간순 정렬 및 해석에 모호함을 야기했습니다.

이러한 문제점을 해결하고 보스 일정 관리의 명확성을 높이기 위해 다음과 같은 입력 규칙이 수립되었습니다.

1.  **날짜 명시:** 보스 목록 텍스트의 시작점과 날짜가 변경되는 각 시점에는 항상 `MM.DD` 형식으로 날짜를 명시합니다 (예: `11.27`, `11.28`).
2.  **순차적 날짜 해석:** 파서는 날짜 표시를 만나면, 다음 날짜 표시가 나타나기 전까지의 모든 보스 시간을 해당 날짜에 할당합니다. 즉, 날짜는 순차적으로 해석되며, 명시적인 날짜 표시 없이 이전 날짜로 "되돌아가지" 않습니다. (예: `11.29`가 표시된 후의 `09:00 보스`는 `11.29 09:00 보스`로 해석됩니다.)

이 규칙은 사용자가 보스 일정을 명확히 관리하고 애플리케이션이 이를 일관되게 처리할 수 있도록 하기 위함입니다.
...
이슈 008 해결 과정과 병행하여 본 이슈의 근본 원인인 데이터 관리 및 파싱 로직을 전면 리팩토링하여 해결했습니다.

1.  **SSOT (Single Source of Truth) 확립:**
    *   `BossDataManager`를 데이터의 유일한 원천으로 삼고, 모든 입력(텍스트, 스케줄러, 계산기)이 이를 기준으로 동작하도록 변경했습니다.
    *   텍스트 영역의 실시간 파싱 로직을 제거하고, 명시적인 "보스 설정 저장" 버튼을 도입하여 데이터 변경 시점을 통제했습니다.

2.  **데이터 재구성 (Reconstruction) 전략:**
    *   데이터 저장 시, 기존의 복잡한 병합 로직 대신 **전체 리스트를 시간순으로 정렬하고 날짜 마커를 새로 삽입하여 재구성**하는 방식을 적용했습니다.
    *   이를 통해 날짜 꼬임, 중복, 누락 문제를 원천적으로 방지했습니다.

3.  **ID 기반 식별 및 업데이트:**
    *   보스 이름 대신 **고유 ID(`data-id`)**를 사용하여 보스 데이터를 식별하고 업데이트하도록 변경했습니다.
    *   동명이인(같은 보스, 다른 시간) 문제와 이름 충돌 문제를 해결했습니다.

4.  **날짜 파싱 로직 개선:**
    *   `boss-parser.js`에서 텍스트 파싱 시, 날짜 헤더(`MM.DD`)가 존재하면 자동 날짜 롤오버(`dayOffset++`)를 수행하지 않도록 로직을 수정했습니다.
    *   이로써 중간에 시간을 수정하여 순서가 바뀌더라도 날짜가 엉뚱하게 넘어가는 버그를 해결했습니다.
