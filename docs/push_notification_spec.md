...
* **기술 스택:** 이 프로젝트는 **바닐라 자바스크립트**로 개발된 클라이언트 측 웹 애플리케이션입니다. 외부 연동 시 환경 설정 및 결정에 참고해 주세요.
* **핵심 전략:**
...
Supabase 프로젝트에서 다음 SQL 스키마를 사용하여 데이터베이스 테이블이 이미 생성되었습니다.

```sql
-- 1. schedules Table (명세서 4.1 기준)
   ...
   COMMENT ON TABLE schedules IS '푸시 알림 스케줄의 메타 데이터를 저장합니다.';
   COMMENT ON COLUMN schedules.schedule_id IS '공유 URL에 사용되는 고유 ID';
   COMMENT ON COLUMN schedules.admin_key IS '스케줄 수정을 위한 관리자 인증 키';
   COMMENT ON COLUMN schedules.boss_data IS '전체 보스 목록의 원본 JSON 데이터';
   COMMENT ON COLUMN schedules.updated_at IS '마지막 수정 시간 (Auto-Sync 비교용)';
   COMMENT ON COLUMN schedules.expires_at IS '스케줄 자동 삭제를 위한 만료 시간 (TTL)';


   -- 2. push_alarms Table (명세서 4.2 기준)
   CREATE TABLE IF NOT EXISTS push_alarms (
       ...
       fire_time_unix BIGINT NOT NULL,
       target_notify_unix BIGINT NOT NULL
   );

   -- Comment on table and columns
   COMMENT ON TABLE push_alarms IS '실제 푸시 알림 발송 예약을 위한 테이블';
   COMMENT ON COLUMN push_alarms.id IS '알람의 고유 ID';
   COMMENT ON COLUMN push_alarms.schedule_id IS '어떤 스케줄에 속한 알람인지 식별';
   COMMENT ON COLUMN push_alarms.boss_name IS '알림에 표시될 보스 이름';
   COMMENT ON COLUMN push_alarms.fire_time_unix IS '실제 보스가 출현하는 시간 (초 단위 Timestamp)';
   COMMENT ON COLUMN push_alarms.target_notify_unix IS '푸시 알림을 발송해야 하는 목표 시간 (초 단위 Timestamp)';


   -- 3. Indexes for performance
   CREATE INDEX IF NOT EXISTS idx_push_alarms_schedule_id ON push_alarms(schedule_id);
   CREATE INDEX IF NOT EXISTS idx_push_alarms_target_notify_unix ON push_alarms(target_notify_unix);
   CREATE INDEX IF NOT EXISTS idx_schedules_expires_at ON schedules(expires_at);

   -- 4. Enable Row Level Security (Good Practice for Supabase)
   ALTER TABLE schedules ENABLE ROW LEVEL SECURITY;
   ALTER TABLE push_alarms ENABLE ROW LEVEL SECURITY;

   -- Allow service_role to do anything.
   CREATE POLICY "Enable all for service-role" ON schedules FOR ALL TO service_role USING (true) WITH CHECK (true);
   CREATE POLICY "Enable all for service-role" ON push_alarms FOR ALL TO service_role USING (true) WITH CHECK (true);
```
