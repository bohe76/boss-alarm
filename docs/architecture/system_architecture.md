# 보스 알리미 시스템 아키텍처 문서 (v3.0.0 - 4-테이블 DB 정규화 아키텍처)

## 1. 개요

본 문서는 "보스 알리미" 웹 애플리케이션의 v3.0.0 시스템 아키텍처를 설명합니다. 이 애플리케이션은 게임 보스 출현 시간을 사용자에게 알리고, 보스 목록을 공유할 수 있는 클라이언트 측 웹 애플리케이션입니다. v3.0에서는 localStorage 기반 직접 배열 관리에서 **4-테이블 정규화 DB(`src/db.js`)** 아키텍처로 전환하여 데이터 무결성과 확장성을 크게 강화했습니다. 보스 데이터는 폼 기반 단일 입력 방식(보스 스케줄러)으로 통일되었으며, 공유 URL은 v3data 포맷(`share-encoder.js`)으로 재설계되었습니다.
...
**백그라운드 알림 처리 및 성능 최적화:** 브라우저의 메인 스레드 부하 나 백그라운드 상태(탭 전환, 최소화)에 영향을 받지 않고 정확한 시간에 알림을 제공하기 위해 **Web Worker (`src/workers/timer-worker.js`)**를 도입했습니다. 특히 v2.17.6에서는 **이벤트 기반 렌더링 아키텍처**를 적용하여 불필요한 메인 스레드 인터벌 루프를 제거하고, 데이터 통신과 렌더링 사이클을 구조적/UI적 용도에 따라 이원화하여 안정성을 극대화했습니다.
...
*   **PiP 위젯 (Picture-in-Picture Widget):** 최신 크롬/엣지 브라우저의 Document Picture-in-Picture API를 활용하여, 대시보드의 '다음 보스' 정보(보스 이름, 남은 시간)를 브라우저 밖으로 꺼내어 게임 화면 위에 항상 떠 있는 미니 창으로 제공합니다. **DOM 실측 기반의 정밀 리사이징(`adjustWindowHeight`)을 통해 렌더링된 콘텐츠의 실제 높이(offsetHeight)를 측정하고 1픽셀 단위로 정확하게 창 크기를 자동 조절하여 최적의 가독성을 제공합니다.** (v2.12+)
*   **보스 시간표:** '보스 시간표' 화면에서 DB에 저장된 보스 스케줄을 조회합니다.
    *   **카드 모드:** 날짜별로 그룹화된 직관적인 카드 형태로 보스 정보를 표시합니다.
    *   **테이블 모드:** 세련된 브라운 테마의 표 형식으로 많은 정보를 한눈에 파악하기 용이합니다.
    *   '다음 보스' 필터 기능을 통해 현재 시간 이후의 보스만 볼 수 있으며, 48시간 내에 보스 데이터가 없는 빈 날짜 헤더는 자동으로 제거(Purification)되어 시각적 노이즈를 차단합니다. 마지막으로 사용한 보기 모드는 로컬 스토리지에 유지됩니다.
    *   **고정 알림 병합**: 시간표 카드·표·텍스트·이미지 내보내기 4경로 모두 `_expandFixedAlarmsInRange()`를 통해 고정 알림을 동일한 shape으로 병합하여 표시합니다.
*   **시간표 내보내기 및 실시간 프리뷰 (v2.17 - UI 격리 아키텍처)**: 텍스트 복사 및 이미지 저장 기능을 제공하며, 안정성을 위해 다음 아키텍처를 적용했습니다.
    *   **UI 격리(Isolation) 원칙:** 내보내기용 이미지 생성은 메인 UI와 **물리적으로 분리된 전용 컨테이너(`export-capture-container`)**에서 수행됩니다. 메인 화면의 레이아웃(2단) 변경이 내보내기 결과물(1단)에 영향을 주거나 그 반대의 경우가 발생하지 않도록 근본적으로 차단합니다.
    *   **스타일 동기화 (Style Sync):** 별도 컨테이너를 사용하지만, **동일한 HTML 구조와 CSS 클래스**를 재사용하여 메인 화면과 100% 동일한 디자인(폰트, 색상, 테두리 등)을 보장합니다. **Inline Style 사용은 엄격히 금지됩니다.**
    *   **모바일 최적화 레이아웃:** 내보내기 이미지는 날짜/필터 조건과 무관하게 항상 가독성이 가장 높은 **1단 레이아웃(Single Column)**으로 렌더링됩니다.
...
*   **공유 가능한 URL:** DB에 저장된 보스 스케줄을 `share-encoder.js`의 `encodeV4Data()`로 URL-safe base64 인코딩하여 `#d=` fragment로 공유합니다(v4 포맷). TinyURL API를 통해 단축 URL을 생성합니다. v3.0.0~v3.0.1에서 발급된 `?v3data=` 링크는 `decodeShareData()`를 통해 영구 호환 수신됩니다. (고정 알림은 공유되지 않습니다.)
...
*   **고정 알림 (Fixed Alarms):** 사용자가 설정한 요일에 따라 반복되는 알림을 제공합니다. 모달을 통해 추가/편집할 수 있으며, 활성/비활성 상태를 개별적으로 관리합니다.
    *   **정밀 정렬 UX**: 설정 리스트 내에서 요일 버튼 뭉치와 편집/삭제 버튼이 **수직 중앙(`align-items: center`)**에 완벽히 정렬되도록 레이아웃을 보정하고, 여백을 최적화하여 조작 편의성을 높였습니다 (v2.18).
...
*   **설정 및 데이터 파일:** `src/data/` 폴더에 `boss-presets.json`, `initial-default.json`, `update-notice.json`, `faq_guide.json`, `feature_guide.json`, `version_history.json`과 같은 설정 및 데이터 파일을 관리합니다.
    *   **v3 4-테이블 정규화 DB (`src/db.js`)**: localStorage를 기반으로 `v3_games`, `v3_bosses`, `v3_schedules`, `v3_settings`, `v3_uid_counter` 5개 키로 데이터를 정규화합니다. `DB.save()`는 QuotaExceededError 발생 시 `false`를 반환하고, `DB.importAll()`은 FK 검증(schedules.bossId → bosses)을 수행합니다. `DB.subscribe()`는 unsubscribe 함수를 반환합니다.
    *   **데이터 관리 (BossDataManager + DB):** 보스 메타데이터(젠 주기 등)는 `boss-presets.json`에서 비동기로 로드하여 `syncPresetsToDb()`로 DB에 동기화합니다. `preset-loader.js`는 `DB.upsertGame`, `DB.upsertBoss`, `DB.deleteBoss`(cascade 정리)를 통해 프리셋 변경 시 DB를 갱신합니다. `BossDataManager._expandAndReconstruct()`는 GC 로직(alerted_0min 완료된 과거 스케줄 제거, 보스별 최소 1개 보존)과 Future Anchor Keeper(미래 인스턴스가 없으면 강제 생성)를 포함합니다.
    *   **고성능 이벤트 알림 및 렌더링**: `BossDataManager`는 알림을 `notifyStructural`(데이터 구조 변경 시)과 `notifyUI`(단순 시간 업데이트 시)로 구분하여 트리거합니다.
    *   **시간 정밀도 유지 및 Time Drift 방지:** 모든 시간 연산은 SSOT의 `scheduledDate`를 절대적인 기준으로 삼습니다.
    *   **침공 보스 필터링**: 프리셋 메타데이터의 `isInvasion` 플래그를 참조하여, 당일이 아닌 침공 보스는 일정 확장 과정에서 자동으로 제외합니다.
    *   **보스 스케줄러 (폼 기반 단일 입력)**: v3에서 텍스트 모드를 완전히 제거하고 폼 기반 입력으로 통일했습니다. 유효성 검사는 '보스 시간 업데이트' 버튼 클릭 시점에만 수행합니다.
...
*   **Local Storage:** `DB` 싱글톤(`src/db.js`)과 `LocalStorageManager`를 통해 사용자 설정(고정 알림, 음소거 상태 등) 및 보스 데이터를 브라우저에 영구적으로 저장합니다. 보스/게임/스케줄 데이터는 v3 4-테이블 구조로, 기타 설정은 `LocalStorageManager`로 분리 관리합니다.
