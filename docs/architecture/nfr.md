...
v3.0.2부터 공유 URL은 `#d=<URL-safe base64>` fragment 방식(v4 포맷)을 사용합니다. fragment는 서버로 전송되지 않으므로 GitHub Pages의 414 URI Too Long 오류를 원천 차단합니다. v3의 `?v3data=` 쿼리스트링은 수신 측에서 영구 호환됩니다.

`decodeShareData()`는 v3/v4를 자동 판별하며, 각각 버전 필드(`v === '3'` 또는 `v === '4'`) 및 타입 검증을 수행합니다. 단, 검증 통과 시 데이터는 DB에 직접 저장되므로, 신뢰할 수 없는 URL 사용 시 보스 일정이 덮어쓰일 수 있습니다.

인코딩 결과가 4000자를 초과하는 경우 `share.js`의 길이 가드가 작동하여 공유 생성을 중단하고 사용자에게 토스트 안내를 표시합니다.
