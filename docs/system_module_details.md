...
### 역할
애플리케이션의 가장 첫 번째 진입점(entry point)입니다. `event-handlers.js`의 `initApp` 함수를 `import`하여 호출하는 단일 책임을 가집니다.

### 주요 로직
`index.html`에서 `<script type="module" src="src/app.js"></script>` 형태로 로드되며, 애플리케이션의 모든 동작을 시작시킵니다.
...
### 역할
`app.js`로부터 호출되는 실질적인 애플리케이션의 메인 컨트롤 타워입니다. 모든 모듈의 생명주기를 관리하고, 전역 UI 이벤트를 처리하며, 화면 전환을 관장하는 최상위 오케스트레이터입니다.

### 주요 `export` 함수

#### `initApp()`
- **설명:** 애플리케이션의 메인 진입점. `app.js`에서 단 한번 호출되어 앱 전체를 초기화하고 실행합니다. 비동기(`async`) 함수입니다.
- **인자:** 없음
- **반환값:** `Promise<void>`
- **핵심 내부 로직:**
    ...
    8.  `initEventHandlers(DOM, globalTooltip)`를 호출하여 전역 UI 이벤트 핸들러 및 **즉시 초기화가 필요한 화면 모듈**들을 등록합니다.
...
- **설명:** `screenId`에 해당하는 화면만 표시하고, 다른 모든 화면은 숨깁니다. 또한 화면 전환에 따른 부가적인 로직을 처리합니다. 특히, 일부 화면(`share`, `version-info`, `help`, `alarm-log` 등)은 이 함수가 호출될 때 관련 로직이 **지연 초기화(lazily initialized)**됩니다.
- **인자:**
...
- **핵심 내부 로직:**
    ...
    4.  `if (screenId === ...)` 분기문을 통해, **해당 화면으로 전환될 때만 필요한** 초기화 함수(예: `initShareScreen`, `initHelpScreen` 등)를 호출하거나 `EventBus` 이벤트를 발행합니다.
...
- **핵심 내부 로직:**
    ...
    2.  `EventBus.on('navigate', (screenId) => showScreen(DOM, screenId))` 리스너를 등록하여, 어떤 모듈이든 `navigate` 이벤트를 발행하면 `showScreen` 함수가 호출되도록 중앙에서 제어합니다. (실제 코드에서는 `initApp`에서 등록)
    3.  `dashboard`, `boss-management`, `calculator`, `boss-scheduler`, `custom-list`, `notifications` 등 **애플리케이션 시작 시점에 즉시 이벤트 핸들러를 등록해야 하는** 주요 화면 모듈들의 `init...Screen()` 함수를 호출합니다.

---

## 3. `src/event-bus.js` (이벤트 버스)
...
## 4. 화면 모듈 (`src/screens/*.js`)

각 화면의 UI 이벤트 핸들러와 뷰(View) 관련 로직을 캡슐화합니다.

| 모듈 파일 | 주요 `export` 함수 | 상세 역할 및 내부 로직 |
|---|---|---|
| **`dashboard.js`** | `initDashboardScreen(DOM)` | '음소거' 버튼의 `click` 이벤트 리스너를 등록합니다. (참고: 대시보드 UI 갱신을 위한 `refresh-dashboard` 이벤트 구독은 이제 `src/global-event-listeners.js`에서 처리합니다.) |
...
| **`notifications.js`** | `initNotificationSettingsScreen(DOM)` | 고정 알림 목록의 이벤트 핸들러(토글, 편집, 삭제)를 위임 방식으로 등록하고, `LocalStorageManager`와 상호작용하여 상태를 변경합니다. |
...
| **`alarm-log.js`** | `initAlarmLogScreen(DOM)`| 화면이 표시될 때 `getLogs`를 호출하여 로그 목록 UI를 생성합니다. (참고: 로그 갱신을 위한 `log-updated` 이벤트 구독은 이제 `src/global-event-listeners.js`에서 처리합니다.) |

---

## 4.1. `src/global-event-listeners.js` (전역 EventBus 리스너 관리)
