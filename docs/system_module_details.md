...
- **핵심 내부 로직:**
    ...
    6.  `renderFixedAlarms(DOM)`, `renderAlarmStatusSummary(DOM)`를 호출하여 초기 UI 상태를 설정합니다.
    7.  `initEventHandlers(DOM, globalTooltip)`를 호출하여 전역 UI 이벤트 핸들러 및 화면별 이벤트 핸들러를 등록합니다.
    8.  `initGlobalEventListeners(DOM)`를 호출하여 전역 `EventBus` 리스너(예: 로그 업데이트 시 알림 로그 화면 갱신)를 등록합니다.
...
## 7. `src/global-event-listeners.js` (전역 EventBus 리스너 관리)
