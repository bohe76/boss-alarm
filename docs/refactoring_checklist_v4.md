...
- [ ] **사전 분석:** 없음 (새 파일 생성).
- [ ] **실행 계획:** `src/utils.js` 경로에 비어 있는 새 파일을 생성합니다.
- [ ] **검증:** 없음.
...
- [ ] **사전 분석:** `ui-renderer.js`의 `formatTimeDifference` 함수 내 `pad` 로직이 다른 외부 변수에 의존하지 않는 순수 로직인지 최종 확인합니다.
- [ ] **실행 계획:** `src/utils.js`에 `padNumber` 함수를 생성하고 `export` 합니다.
    ```javascript
    // src/utils.js
    /**
...
- [ ] **사전 분석:** `app.js`(`event-handlers.js`)의 `initApp` 함수 내에서 `formatMonthDay`가 사용하는 `date` 객체 외에 다른 지역 변수에 의존하지 않는지 최종 검증합니다. `utils.js`에 `padNumber`가 먼저 추가되었는지 확인합니다.
- [ ] **실행 계획 1 (이전/리팩토링):** `app.js`의 `initApp` 내 `formatMonthDay` 로직을 `src/utils.js`로 옮기고, `padNumber`를 사용하도록 수정합니다.
- [ ] **실행 계획 2 (가져오기):** `src/app.js` 파일 상단에 `import { formatMonthDay } from './utils.js';`를 추가합니다.
- [ ] **실행 계획 3 (삭제):** `src/app.js`의 `initApp` 내에서 기존 `const formatMonthDay = ...` 라인을 삭제합니다.
...
- [ ] **사전 분석:** `ui-renderer.js`와 `app.js` 양쪽에 정의된 `isValidTime` 함수의 내용이 100% 동일한지, 미묘한 차이는 없는지 최종 확인합니다.
- [ ] **실행 계획 1 (생성):** `src/utils.js`에 `isValidTime` 함수를 추가하고 `export` 합니다.
- [ ] **실행 계획 2 (삭제/가져오기):** `src/ui-renderer.js`에서 기존 함수를 삭제하고, `utils.js`에서 `import` 합니다.
- [ ] **실행 계획 3 (삭제/가져오기):** `src/app.js`에서 기존 함수를 삭제하고, `utils.js`에서 `import` 합니다.
