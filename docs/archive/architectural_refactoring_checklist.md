...
- [x] **사전 분석:** 없음 (새 파일 생성).
- [x] **실행 계획:** `src/router.js` 경로에 아래 내용으로 새 파일을 생성합니다.
    ```javascript
    // src/router.js
    ...
    ```
- [x] **검증:** 없음.
- [x] **커밋 제안:** `feat(core): 중앙 화면 관리를 위한 라우터 모듈 생성`
</details>

<details>
<summary><strong>1.2. 화면 모듈에 `getScreen` 함수 추가 및 라우터에 등록</strong></summary>

- [x] **사전 분석:** `screens` 디렉토리 내의 모든 화면 모듈 파일 목록을 확인합니다. 각 파일은 `init...Screen` 또는 유사한 초기화 함수를 `export` 하고 있습니다.
- [x] **실행 계획:**
...
- [x] **검증:** 모든 화면 모듈에 `getScreen` 함수가 추가되고, 라우터에 정상적으로 등록되는지 확인합니다. (기능 변경 없음)
- [x] **커밋 제안:** `feat(screens): 각 화면 모듈에 메타 정보 반환 함수 추가 및 라우터 등록`
</details>

<details>
<summary><strong>1.3. `event-handlers.js`의 `showScreen` 함수를 라우터 사용하도록 리팩토링</strong></summary>

- [x] **사전 분석:** `app.js`의 `showScreen` 함수 내부에 있는 긴 `if (screenId === ...)` 분기문을 확인합니다. (참고: `showScreen` 함수는 `app.js`로 이전되었습니다.)
- [x] **실행 계획:**
...
- [x] **검증:** 애플리케이션의 모든 메뉴(사이드바, 하단 탭)를 클릭하여 모든 화면이 이전과 같이 정상적으로 표시되고 기능하는지 전체 회귀 테스트를 수행합니다.
- [x] **커밋 제안:** `refactor(core): showScreen이 중앙 라우터를 사용하도록 변경`
</details>
...
- [x] **사전 분석:** `initApp` 함수에서 `initLogger`, `loadBossLists`, `LocalStorageManager.init`, `CustomListManager.init` 등 서비스 초기화와 관련된 코드를 식별합니다.
- [x] **실행 계획:**
...
- [x] **주의사항:** `initializeCoreServices` 내에서 초기화되는 `LocalStorageManager`, `CustomListManager` 등 데이터 관리자들은 `initApp`에서 `BossDataManager.subscribe` 전에 완전히 초기화되어야 합니다.
- [x] **검증:** 앱을 새로고침했을 때, 로그, 보스 목록, 로컬 스토리지 데이터가 모두 정상적으로 로드되는지 확인합니다.
- [x] **커밋 제안:** `refactor(core): 핵심 서비스 초기화 로직을 services.js로 분리`
</details>

<details>
<summary><strong>2.2. 초기 데이터 로딩 및 파싱 로직 분리</strong></summary>

- [x] **사전 분석:** `app.js`의 `initApp` 함수에서 URL 파라미터를 파싱하고, 기본 보스 목록을 설정하며, `parseBossList`를 호출하는 부분을 식별합니다.
- [x] **실행 계획:**
...
- [x] **커밋 제안:** `refactor(core): 초기 데이터 로딩 및 파싱 로직 함수로 분리`
</details>
...
- [x] **사전 분석:** `BossDataManager` 객체의 구조를 확인합니다.
- [x] **실행 계획:**
...
- [x] **검증:** 없음 (내부 구조 변경).
- [x] **주의사항:** `BossDataManager`의 `subscribe` 메소드를 사용하는 모듈(예: `app.js`)에서는 `import { BossDataManager } from './data-managers.js';`와 같이 명시적으로 임포트해야 합니다.
- [x] **커밋 제안:** `feat(state): 데이터 변경 감지를 위한 구독/알림 패턴 추가`
</details>

<details>
<summary><strong>3.2. 대시보드 자동 갱신 리팩토링</strong></summary>

- [x] **사전 분석:** `EventBus.on('refresh-dashboard', ...)` 리스너와, 이 이벤트를 발생시키는 모든 `EventBus.emit('refresh-dashboard')` 호출 코드를 식별합니다. (`alarm-scheduler.js`, `app.js` 등)
- [x] **실행 계획:**
...
- [x] **커밋 제안:** `refactor(state): EventBus 기반의 수동 갱신을 구독 패턴으로 변경`
</details>
...
- [x] **사전 분석:** `screens/help.js`의 `initHelpScreen` 함수 내에서 `helpData`를 받아 HTML 문자열을 동적으로 생성하는 부분을 식별합니다.
- [x] **실행 계획:**
...
- [x] **검증:** '도움말' 화면으로 이동했을 때, 이전과 동일하게 아코디언 메뉴가 정상적으로 표시되고 작동하는지 확인합니다.
- [x] **커밋 제안:** `refactor(ui): 도움말 화면 렌더링 로직을 ui-renderer로 이전`
</details>

<details>
<summary><strong>4.2. `version-info.js`의 렌더링 로직을 `ui-renderer.js`로 이전</strong></summary>

- [x] **사전 분석:** `screens/version-info.js`의 `initVersionInfoScreen` 함수가 `renderVersionInfo`를 호출하고, 이 `renderVersionInfo`가 `ui-renderer.js`에 이미 존재하며 HTML 생성 로직을 포함하고 있음을 확인합니다. (이 구조는 이미 잘 분리되어 있으나, 일관성을 위해 호출 흐름을 명확히 합니다.)
- [x] **실행 계획:** (`version-info.js`는 이미 `ui-renderer.js`의 함수를 호출하는 좋은 구조를 가지고 있으므로, 만약 자체 렌더링 로직이 있다면 이전합니다. 현재 구조에서는 코드 변경이 거의 필요 없을 수 있습니다. 재확인 후 필요 시 아래 계획 실행)
...
- [x] **검증:** '릴리즈 노트' 화면으로 이동했을 때, 버전 정보가 이전과 동일하게 표시되는지 확인합니다.
- [x] **커밋 제안:** `refactor(ui): 릴리즈 노트 렌더링 책임 ui-renderer로 완전 위임`
</details>