...
*   **'다음 보스' 표시 정보 레이아웃 및 가독성 개선:**
    *   보스 이름이 길어질 때 '남은 시간'이 다음 줄로 넘어가는 문제를 해결하기 위해, '보스 이름' 뒤에 `<br>` 태그를 추가하여 '남은 시간'을 항상 새로운 줄에 표시하도록 HTML 구조를 변경했습니다. (`src/ui-renderer.js`)
    *   '남은 시간' 정보의 가독성을 높이기 위해 괄호 `()`를 제거했습니다. (`src/ui-renderer.js`)
    *   '남은 시간'이 대시보드에서 가장 중요한 정보임을 고려하여 폰트 크기를 `2.0em`으로 확대하여 시각적 강조를 높였습니다. (`src/style.css`)
```

**New log entries to append:**

```markdown
*   **'다가오는 보스' 목록 스타일링 로직 재정리 및 색상 조정:**
    *   사용자 피드백을 반영하여 '다가오는 보스' 목록의 스타일링 로직을 3단계 조건 (기본/1시간 미만/5분 미만)으로 명확히 재정리했습니다. (`src/ui-renderer.js`, `src/style.css`)
    *   **기본 상태 (1시간 이상):** '출현 시간', '보스 이름', '남은 시간'을 모두 회색(`#777`)으로 표시하도록 수정했습니다.
        *   이전 `#999` 색상이 너무 연하다는 피드백에 따라 `#777`로 색상을 변경했습니다.
        *   CSS 우선순위 문제 해결을 위해 `#upcomingBossListContent .spawn-time.default-grey` 규칙에 `!important`를 적용하여 '출현 시간'의 색상이 올바르게 적용되도록 강제했습니다.
    *   **1시간 미만 (5분 이상 ~ 1시간 미만):** '출현 시간', '보스 이름', '남은 시간'을 모두 검은색(`#111`), 굵게 표시하도록 했습니다.
    *   **5분 미만:** 기존의 빨간색/청록색 굵게 표시 로직을 유지했습니다.
    *   코드 정리를 위해 `src/style.css`에서 불필요한 클래스(`.long-remaining`, `.default-view-bold`)를 제거하고, `default-grey`, `medium-priority` 클래스로 대체하여 스타일 구조를 간소화했습니다.

*   **'다음 보스' 보스 이름 색상 변경:**
    *   '다음 보스' 섹션의 보스 이름 색상을 빨간색(`ea4335`)에서 보라색 계열(`#AD1457`)로 변경했습니다. (`src/style.css`)

*   **'다가오는 보스' 5분 미만 보스 이름 색상 변경:**
    *   '다가오는 보스' 목록에서 5분 미만인 보스의 이름 색상을 빨간색(`ea4335`)에서 보라색 계열(`#AD1457`)로 변경하여 '다음 보스'와 일관성을 유지했습니다. (`src/style.css`)

*   **대시보드 '알림 상태' 및 '소리 설정' 색상 변경:**
    *   대시보드의 '알림 상태' 텍스트와 '소리 설정' SVG 아이콘(원 포함)의 빨간색을 '남은 시간'과 동일한 청록색(`#43AA8B`)으로 변경했습니다. (`src/style.css`)

*   **대시보드 '알림 상태' 텍스트 스타일 조정:**
    *   '알림 상태' 텍스트(`DOM.alarmStatusText`)가 ON/OFF 상태에 관계없이 항상 굵게(bold) 표시되도록 변경했습니다. (`src/style.css`)
    *   '알림 상태' 내용의 중요도를 고려하여 폰트 크기를 `1.4em`으로 조정하여 시각적 강조를 높였습니다. (`src/style.css`)
```

**Full content for `docs/ui_gui_improvements_log.md`:**
```markdown
# UI/GUI 개선 및 카드 시스템 통일 (1차) 문서
