...
*   **보스 시간표:** '보스 시간표' 화면에 **뷰 모드**와 **편집 모드**를 제공합니다.
    ...
    *   **뷰 모드:** 보스 목록을 **날짜별 카드 리스트 형태**로 시각적으로 명확하게 표시하며, '다음 보스' 필터 기능을 통해 현재 시간 이후의 보스만 볼 수 있습니다. 일반 길드원들이 보스 목록을 확인하기에 용이합니다.
...
*   **설정 및 데이터 파일:** `src/data/` 폴더에 `boss-presets.json` (보스별 젠 주기 및 프리셋), `initial-default.json` (초기 표시 목록), `faq_guide.json`, `feature_guide.json`, `version_history.json`과 같은 설정 및 데이터 파일을 관리합니다.
    ...
    *   **데이터 관리 (SSOT & Draft 패턴):**
        ...
        *   **시간 정밀도 유지 및 Time Drift 방지:** 모든 시간 연산은 SSOT의 `scheduledDate`를 절대적인 진실(Truth)로 삼습니다. UI 렌더링 시 분 단위 남은 시간에서 역계산하는 방식 대신, SSOT의 정밀한 시점(Date 객체)을 직접 출력하여 누적 오차를 차단합니다.
        *   **이름 기반 매칭(Name-based Matching):** 간편 입력 모드에서는 보스 이름을 키로 데이터를 매칭합니다. 동일한 이름의 보스가 여러 개 존재할 경우, 현재 시각에서 가장 가까운 미래 또는 과거 시점의 보스를 지능적으로 선택하여 대시보드와 동기화합니다.
