...
*   **보스 목록 관리:** '보스 설정' 화면에 **뷰 모드**와 **편집 모드**를 제공합니다.
    *   **편집 모드:** 사용자는 텍스트 영역에 `HH:MM 보스이름` 형식으로 보스 출현 시간과 이름을 입력할 수 있습니다. 입력 후 "보스 설정 저장" 버튼을 클릭하여 명시적으로 저장해야 하며, 이때 데이터 파싱, 유효성 검사, 정렬이 수행되어 데이터 무결성을 보장합니다.
