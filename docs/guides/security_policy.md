...
단, **공유 URL(`#d=` fragment 또는 v3 호환 `?v3data=` 파라미터)로 수신된 데이터**는 잠재적으로 타인이 생성한 외부 입력이다. 이 데이터는 `share-encoder.js`의 `decodeShareData()`(v3/v4 자동 판별)로 파싱된 후 DB에 저장되고, 이후 동적으로 렌더링된다. 이 경로는 **실질적 XSS 위험이 존재**한다.
