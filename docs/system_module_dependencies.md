...
| **`boss-management.js`**| `boss-parser.js`, `ui-renderer.js`, `data-managers.js`, `logger.js` | '뷰/편집' 모드 토글, '다음 보스' 필터 토글, `parseBossList()`, `updateBossManagementUI()`, `renderBossListTableView()`, `updateBossListTextarea()`, `BossDataManager.setBossSchedule()`, `log()` |
