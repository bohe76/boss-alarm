...
| **`boss-management.js`**| `boss-parser.js`, `ui-renderer.js`, `data-managers.js`, `logger.js` | '뷰/편집' 모드 토글(`LocalStorageManager` 사용), '다음 보스' 필터 토글(`LocalStorageManager` 사용), `parseBossList()`, `updateBossListTextarea()`, `updateBossManagementUI()`, `renderBossListTableView()`, `BossDataManager.setBossSchedule()`, `log()` |
