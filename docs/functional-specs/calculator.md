...
*   **보스 목록 업데이트:**
    ...
    *   **정밀 날짜/시간 보존 엔진 (v2.17.4)**: 화면에 표시된 텍스트(`HH:MM:SS`)를 파싱하지 않고, 계산 시점에 메모리에 저장해 둔 **정밀한 `Date` 객체(`lastCalculatedBossTime`)를 직접 참조**하여 업데이트합니다. 이를 통해 자정을 넘기는 계산(오늘 23시 + 2시간 = 내일 01시) 시 날짜 정보가 유실되거나 '오늘'로 잘못 기록되는 문제를 원천 차단합니다. 또한 `BossDataManager.setBossSchedule`에 위임하여 전체 일정을 정규화(Normalization)하고 재구성합니다.
