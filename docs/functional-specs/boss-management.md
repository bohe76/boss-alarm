...
**6. 뷰 모드 (View Mode)**
*   **개요:** 보스 목록을 편집하지 않고 확인만 하는 사용자를 위한 읽기 전용 모드입니다. 텍스트 영역 대신 날짜별 카드 리스트 형태로 보스 목록을 직관적으로 보여줍니다.
*   **다음 보스 필터:**
    *   화면 상단의 '다음 보스' 토글 버튼을 통해 활성화할 수 있습니다.
    *   활성화 시, 현재 시간 이후에 출현하는 보스들만 목록에 표시됩니다.
    *   **자동 갱신:** 필터가 활성화된 상태에서는 1초마다 목록이 자동으로 갱신되어, 시간이 지난 보스는 목록에서 실시간으로 사라집니다. 이를 통해 사용자는 항상 유효한 보스 목록만 확인할 수 있습니다.
*   **상태 저장:** 뷰 모드 사용 여부와 다음 보스 필터의 활성화 상태는 로컬 스토리지에 저장되어, 나중에 앱을 다시 열어도 동일한 상태가 유지됩니다.
