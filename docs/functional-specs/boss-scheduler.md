...
**5. 보스 설정 적용**
*   **SSOT 원칙:** 스케줄러는 **Draft(임시 SSOT)**를 기반으로 동작합니다. 화면 진입 시 Main SSOT에서 Draft를 복사하고, 모든 편집은 Draft에 반영됩니다. "보스 설정 적용" 시 Draft가 Main SSOT에 커밋됩니다.
*   **입력/텍스트 모드:**
    *   **입력 모드:** 보스 목록 드롭다운에서 선택한 프리셋을 기반으로 입력 폼이 생성됩니다. 각 보스에 대해 "남은 시간"을 입력하면 젠 시간이 자동 계산됩니다.
    *   **텍스트 모드:** 전문가 모드로, 사용자가 직접 텍스트로 보스 목록을 입력합니다. `#`으로 비고를 구분할 수 있습니다. Draft 전체 내용이 텍스트로 표시됩니다.
    *   **모드 전환:** 입력 모드 → 텍스트 모드 전환 시 유효한 입력만 Draft에 반영됩니다. 유효한 입력이 없으면 기존 Draft가 유지됩니다.
*   **트리거:** 모든 보스에 대한 입력 완료 후 '보스 설정 적용' 버튼(`moveToBossSettingsButton`)을 클릭하면 현재 Draft를 Main SSOT에 커밋합니다.
*   **데이터 처리:**
    ...
    4.  **\"침공\" 보스 당일 필터링:** 이름에 **\"침공\"**이라는 단어가 포함된 *모든* 보스의 경우, 계산된 젠 시간이 *현재 날짜*에 속하지 않으면 해당 보스 항목은 목록에서 자동으로 제외됩니다.
    5.  구성된 보스 데이터는 `BossDataManager.commitDraft()` 함수를 통해 Main SSOT에 반영됩니다. 이 과정에서 전체 보스 목록은 시간 순으로 정렬되고 날짜 마커가 자동으로 삽입됩니다.
*   **날짜 롤오버 처리:** 텍스트 파싱 시 **시간 역전 감지 로직**이 적용됩니다. 이전 보스보다 시간이 이른 보스가 나타나면 날짜를 다음 날로 자동 증가시킵니다. (예: 23:29 → 03:50은 다음 날)
