...
**5. 보스 시간 업데이트**
*   **SSOT 원칙:** 스케줄러는 **Draft(임시 SSOT)**를 기반으로 동작하며, **scheduledDate(절대 시점)**를 데이터의 유일한 진실로 간주합니다.
    *   **정밀도 유지:** 화면 렌더링 시 Draft의 `scheduledDate`를 직접 읽어 출력하므로, 분 단위 남은 시간 역계산으로 인한 오차가 발생하지 않습니다.
    *   **이름 기반 매칭 및 중복 제거**: 간편 입력 모드에서는 보스 이름을 키로 사용하며, **48시간 확장 데이터 중 각 보스별로 현재와 가장 가까운 앵커 하나만 노철되도록 중복을 제거(Set 사용)**하여 UI 가독성을 확보합니다. 동일 이름 존재 시 현재 시각 기준 가장 가까운 미래 항목을 우선 매칭하며, 미래 데이터가 없을 경우 가장 최근의 과거 데이터를 앵커로 선택합니다.
*   **입력/텍스트 모드:**
    *   **간편 입력 모드:** 보스 목록 드롭다운에서 선택한 프리셋을 기반으로 입력 폼이 생성됩니다. 각 보스에 대해 "남은 시간"을 입력하면 젠 시간이 자동 계산됩니다.
    *   **텍스트 모드:** 전문가 모드로, 사용자가 직접 텍스트로 보스 목록을 입력합니다.
        *   **필수 규칙**: **첫 번째 줄은 반드시 'MM.DD' 형식의 날짜**로 시작해야 합니다. (예: `12.30`)
        *   **구분자**: 보스 이름과 비고(메모)는 `#`으로 구분할 수 있습니다. (예: `12:30 파르바 # 7번 입구`)
        *   Draft 전체 내용이 텍스트로 표시됩니다.
    *   **모드 전환:** 간편 입력 모드 ↔ 텍스트 모드 전환 시 변경 사항이 Draft에 상호 반영됩니다.
*   **트리거:** 모든 보스에 대한 입력 완료 후 '보스 시간 업데이트' 버튼을 클릭하면 현재 Draft를 Main SSOT에 커밋합니다.
*   **데이터 처리 및 자동 확장:**
    1.  각 보스의 "남은 시간" 입력 필드에 있는 값과 계산된 "젠 시간"을 기반으로 보스 데이터를 구성합니다.
    2.  **48시간 일정 자동 확장 (핵심)**: `BossDataManager`는 입력된 보스 시간을 기준으로 **'오늘 00:00 ~ 내일 23:59'**까지 총 48시간의 일정을 보스별 젠 주기에 맞춰 자동으로 생성합니다.
    3.  이때, `data-id` 속성을 통해 기존 보스에 해당하는 ID를 유지하여 `BossDataManager`와의 병합 시 기존 알림 상태가 보존되도록 합니다.
    4.  **"침공" 보스 당일 필터링:** 이름에 **"침공"**이라는 단어가 포함된 보스의 경우, 계산된 젠 시간이 *현재 날짜*에 속하지 않으면 해당 보스 항목은 목록에서 자동으로 제외됩니다.
    5.  구성된 보스 데이터는 `BossDataManager.commitDraft()` 함수를 통해 Main SSOT에 반영됩니다. 이 과정에서 전체 보스 목록은 시간 순으로 정렬되고 날짜 마커가 자동으로 삽입됩니다.
*   **날짜 롤오버 처리:** 텍스트 파싱 시 **시간 역전 감지 로직**이 적용됩니다. 이전 보스보다 시간이 이른 보스가 나타나면 날짜를 다음 날로 자동 증가시키며, 이 역시 48시간 확장 엔진에 의해 정규화됩니다.
*   **화면 전환:** 보스 시간 업데이트가 완료되면, 자동으로 '보스 시간표' 화면으로 이동하여 사용자가 변경된 보스 목록을 확인할 수 있도록 합니다.
