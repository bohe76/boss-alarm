...
*   **UI 상태 제어:** 
    - **임계값 기반 알림**: 다음 보스를 제외한 **후속 다가오는 보스들** 중 남은 시간이 5분 미만(`IMMINENT`)인 보스가 존재할 경우, PiP 창 내의 **Bell-Alert 아이콘**을 활성화하고 깜빡여 가려진 임박 보스의 존재를 알립니다. (다음 보스는 숫자의 색상으로 상태를 이미 알리고 있으므로 중복을 피함)
...
1.  **노출 여부 판단**: `app.js: initEventHandlers`에서 현재 `window.APP_VERSION`(예: 2.17.0)을 기준으로 `hide_update_modal_2.17.0` 키가 로컬 스토리지에 존재하는지 확인합니다.
